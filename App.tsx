...
function App() {
  ...
  return (
    <div className="w-screen h-screen bg-[#1f1212] text-white flex flex-col relative overflow-hidden font-sans selection:bg-[#E50914] selection:text-white">
      ...
      <main className="flex-1 relative w-full h-full pt-16 overflow-visible">
        <AnimatePresence initial={false} custom={direction} mode="wait">
          <motion.div
            key={currentSlide}
            custom={direction}
            variants={{
              enter: (direction: number) => ({ opacity: 0 }),
              center: { zIndex: 1, opacity: 1 },
              exit: (direction: number) => ({ zIndex: 0, opacity: 0 })
            }}
            initial="enter"
            animate="center"
            exit="exit"
            transition={{ duration: 0.4, ease: "easeInOut" }}
            className="absolute inset-0 w-full h-full bg-transparent overflow-visible"
          >
      ...
      <div className="absolute bottom-8 w-full z-40 hidden md:flex justify-center pointer-events-none">
        <div className="flex flex-col items-center gap-1 opacity-40">
          <p className="text-[10px] font-mono uppercase tracking-[0.4em] text-[#E50914]/80 text-shadow-sm">Now Playing</p>
          <p className="text-[10px] font-sans uppercase tracking-[0.2em] text-[#ffccaa] drop-shadow-md">AI Short-Form Video Creator Course</p>
        </div>
      </div>
    </div>
  );
